#include <iostream>
#include <iomanip>
#include <cmath>
#include <fstream>

using namespace std;

struct Point
{
	long long x, y;
	
	long long operator *(const Point &a)
	{
		return x * a.x + y * a.y;
	}
	
	long long operator % (const Point &a)
	{
		return x * a.y - y * a.x;
	}
	
	Point operator - (const Point &a)
	{
		return Point{x - a.x, y - a.y};
	}
};


int f(Point p1, Point p2, Point p3, Point p4)
{
	Point v1 = p1 - p3;
	Point v2 = p2 - p3;
	Point v3 = p4 - p3;
	if (((v1 % v3) < 0 && (v2 % v3) > 0) || ((v1 % v3) > 0 && (v2 % v3) < 0))
		return 1;
	else 
		return 0;
}

int f1(Point p1, Point p2, Point p3)
{
	Point v1 = p1 - p2;
	Point v = p3 - p2;
	Point v2 = p1 - p3;
	if ((v1 * v) * (v2 * v) <= 0 && (v1 % v == 0))
		return 1;
	else
		return 0;
}

bool segmentIntersec(Point &p1, Point &p2, Point &p3, Point &p4)
{
	if (f(p1, p2,  p3, p4) + f(p3, p4, p1, p2) == 2)
		return true;
	else
	{
		if (f1(p1, p3, p4) + f1(p2, p3, p4) + f1(p3, p1, p2) + f1(p4, p1, p2) > 0)
			return true;
		else
			return false;
	}
}

double dist(Point &p1, Point &p2)
{
	return sqrt((p1.x - p2.x) * (p1.x - p2.x) + (p1.y - p2.y) * (p1.y - p2.y));
}

double dist(Point &p1, Point &p2, Point &p3)
{
//	Point p1{x2 - x1, y2 - y1};
//	Point p2{x3 - x1, y3 - y1};
//	Point p3{x3 - x2, y3 - y2};
//	Point p4{x2 - x3, y2 - y3};
	Point v1 = p2 - p1;
	Point v2 = p3 - p1;
	Point v3 = p3 - p2;
	Point v4 = p2 - p3;
	double res = 0;
	if (v1 * v3 < 0 && v2 * v4 < 0)
		res = (1.0 * abs((v1 % v2))) / (dist(p2, p3));
	else
		res = min(dist(p1, p2), dist(p1, p3));
	return res;
}

int main(int argc, char *argv[])
{
	ifstream fin("distance5.in");
	ofstream fout("distance5.out");
	int x1, y1, x2, y2, x3, y3, x4, y4;
	fin >> x1 >> y1 >> x2 >> y2 >> x3 >> y3 >> x4 >> y4;
	Point p1{x1, y1};
	Point p2{x2, y2};
	Point p3{x3, y3};
	Point p4{x4, y4};
	if (segmentIntersec(p1, p2, p3, p4))
		fout << 0;
	else
	{
		fout << setprecision(20) << min(dist(p1, p3, p4), min(dist(p2, p3, p4), min(dist(p3, p1, p2), dist(p4, p1, p2))));
	}
	return 0;
}
